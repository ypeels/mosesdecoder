...
void RuleLengthPhraseProperty::Populate(const set< vector<string> > &indices, float count)
{
...
void RuleLengthPhraseProperty::Populate(const std::vector<size_t> &toks, float count)
{
  size_t ntInd = toks[0];
...
void RuleLengthPhraseProperty::Populate(Map &map, size_t span, float count)
{
  Map::iterator iter;
  iter = map.find(span);
  if (iter != map.end()) {
...
void RuleLengthPhraseProperty::CalcTotals(Vec &vec)
{
...
float RuleLengthPhraseProperty::GetProb(size_t ntInd, size_t sourceWidth, float smoothing) const
{
	float count;
...
bool RuleLengthPhraseProperty::OnlyContainSpanLength(size_t ntInd, size_t spanLength) const
{
	const std::pair<Map, float> &data = m_source[ntInd];
	const Map &map = data.first;
