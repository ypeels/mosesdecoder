...
sub define_splitter_train {
    ...
    my $input_training_settings = &get("SPLITTER:input-training-settings");
    my $output_training_settings = &get("SPLITTER:output-training-settings");
    
    my $cmd = "";
    if ($input_splitter) {
	$cmd .= "$input_splitter -train -model $output.$input_extension -corpus $input.$input_extension $input_training_settings \n";
    }
    if ($output_splitter) {
